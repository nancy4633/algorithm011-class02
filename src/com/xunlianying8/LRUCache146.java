...
class LRUCache146 {
    ...
    public LRUCache146(int capacity) {
        ...
        head = new DLinkNode();// 因为head和tail，所以DLinkedNode的构造函数一定有一个不包含参数的，因为head和tail都没有key value
        tail = new DLinkNode();
        head.next = tail;
        tail.prev = head;
    }
    ...
    private void addToHead(DLinkNode node) {
        node.prev = head;
        node.next = head.next;
        head.next.prev = node; // 这两句的顺序要千万记住！！！
        head.next = node; // 这两句的顺序要千万记住！！！
    }
