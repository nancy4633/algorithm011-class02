...
public class Combine77 {
    ...
    public List<List<Integer>> combine(int n, int k) {
        ...
        dfs(1, n, k, new ArrayList<>(), results); // 此处n从1开始，不是0！！！//又一次发生，一定要仔细审题
        return results;
    }

    private void dfs(int index, int n, int k, ArrayList<Integer> result, List<List<Integer>> results) {
        if (result.size() == k) {
            results.add(new ArrayList<>(result));
            return; // 出现结果后不继续
        }
        for (int i = index; i <= n - (k - result.size()) + 1; i++) { // 再次记住！！！ 此处的等号要记清楚；此处的计算要清楚，本办法也ok～
            result.add(i);
            // 此处传递了result，所有用i，如果不传递，像permute就需要用index！！！
            dfs(i + 1, n, k, result, results); // 这里是i+1，index只是边界值，因为不允许有重复元素，所以每一次递归，都是当前的i值加一，而不是边界值index+1
