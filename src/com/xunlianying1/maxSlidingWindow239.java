...
public class maxSlidingWindow239 {
    /**
     * 双指针： 解法33的另一种写法，比较容易懂，但是解法一写的更简洁
     *
     * @param nums
     * @param k
     * @return
    ...
    public int[] maxSlidingWindow3(int[] nums, int k) {
        int len = nums.length;
        if (len * k == 0) return nums;
        int[] left = new int[len], right = new int[len], result = new int[len - k + 1];
        for (int i = 0; i < len; i++) {
            if (i % k == 0) left[i] = nums[i];
            else left[i] = Math.max(nums[i], left[i - 1]);
        }
        for (int i = len - 1; i >= 0; i--) {
            if (i % k == 0 || i == len - 1) right[i] = nums[i];
            else right[i] = Math.max(nums[i], right[i + 1]);
        }
    ...
    public int[] maxSlidingWindow33(int[] nums, int k) {
        ...
        int[] left = new int[n], right = new int[n], result = new int[n - k + 1];
        left[0] = nums[0];
        right[n - 1] = nums[n - 1];
        ...
        for (int m = n - k; m >= 0; m++) {
            result[m] = Math.max(right[m], left[m + k - 1]);
        }
        return result;
    }
    ...
    public int[] maxSlidingWindow0(int[] nums, int k) {
        ...
        PriorityQueue<Integer> queue = new PriorityQueue<>((o1, o2) -> (o2 - o1)); // 用法很赞，但是超时了。哈哈哈
        for (int i = 0; i < len; i++) {
    ...
    public int[] maxSlidingWindow1(int[] nums, int k) {
        int len = nums.length, max = (int) Double.NEGATIVE_INFINITY, max_index = -1, left = 0, right = k - 1; // right的初始值要记住！
        int[] result = new int[len - k + 1]; // length的默认值要注意！！！
        while (right < len) {
            if (max_index < left) {
